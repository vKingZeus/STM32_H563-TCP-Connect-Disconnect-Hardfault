...
#define PRINT_IP_ADDRESS(addr)             do { \
                                                sprintf(STM32_IP, " %lu.%lu.%lu.%lu \r", \
                                                (addr >> 24) & 0xff, \
                                                (addr >> 16) & 0xff, \
                                                (addr >> 8) & 0xff, \
                                                addr& 0xff);\
...
#define PRINT_IP_PC(addr)             do { \
                                                sprintf(IP_PC, " %lu.%lu.%lu.%lu \r", \
                                                (addr >> 24) & 0xff, \
                                                (addr >> 16) & 0xff, \
                                                (addr >> 8) & 0xff, \
                                                addr& 0xff);\
...
/* USER CODE BEGIN EFP */
extern void eth_uartDisplay(char *_ethBuffer);
extern void eth_tcpDisplay(char *_out);
extern void ethernet_parserFunction(void);

extern ULONG check_ethConnection(void);
/* USER CODE END EFP */
...
#define DEFAULT_MESSAGE          "\r Detecteur Ethernet \r\n"/
#define TCP_CONNECTED            "\r\nConnexion Ethernet Etablie \r\n"
#define TCP_DISCONNECTED         "\r\nConnexion Perdue \r\n"
#define TCP_LISTENING            "\nTCP Server listening on PORT \r\n"
#define STM_IPADDRESS               "STM32 IpAddress:"

extern int g_nConnected;
/* USER CODE END 1 */
